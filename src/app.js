...
<form class="d-flex me-2" method="get" action="/${cur}:search">
<input class="form-control me-2" name="q" type="search" placeholder="Search" aria-label="Search" value="${search_text}" required>
<button class="btn ${UI.search_button_class}" type="submit">Search</button>
</form>
<button id="theme-toggle" class="btn btn-sm btn-outline-secondary ms-1" onclick="toggleTheme()" title="Toggle dark/light mode">&#9788;</button>
</div>
</div>
</nav>
`;
...
function list(path, id = '', fallback = false) {
  ...
  <div id="folder-filter-wrap" class="mb-2">
    <input id="folder-filter" class="form-control form-control-sm" type="search" placeholder="Filter files..." autocomplete="off">
  </div>
  <div id="list-header" class="list-group-item d-flex align-items-center py-1 small text-muted" style="border-bottom:1px solid rgba(128,128,128,.2);">
    <span class="flex-grow-1 gdi-sort-header" data-sort="name">Name</span>
    <span class="gdi-sort-header me-3" style="min-width:80px;text-align:right;" data-sort="size">Size</span>
    <span class="gdi-sort-header" style="min-width:130px;text-align:right;" data-sort="date">Modified</span>
  </div>
...
function initFolderFilter() {
    ...
    input.addEventListener('input', function() {
        ...
        const items = document.querySelectorAll('#list .list-group-item');
        let shown = 0;
        items.forEach(el => {
...
function sortFileList(col, dir) {
    ...
    items.sort((a, b) => {
        const av = ($(a).data(col) || $(a).data('name') || '').toString().toLowerCase();
        const bv = ($(b).data(col) || $(b).data('name') || '').toString().toLowerCase();
        if (col === 'size') {
            return dir * ((parseFloat($(a).data('bytes')) || 0) - (parseFloat($(b).data('bytes')) || 0));
        }
        if (col === 'date') {
            return dir * (new Date($(a).data('date')) - new Date($(b).data('date')));
        }
...
function append_files_to_list(path, files) {
    ...
    for (const i in files) {
        ...
        // replace / with %2F
        if (item['mimeType'] == 'application/vnd.google-apps.folder') {
            html += `<a href="${p}" style="color: ${UI.folder_text_color};" class="countitems list-group-item list-group-item-action gdi-file-item" data-name="${item.name.toLowerCase()}" data-date="${item['modifiedTime'] || ''}"> ${folder_icon} ${item.name} ${UI.display_time ? `<span class="badge bg-info float-end"> ` + item['modifiedTime'] + ` </span>` : ``}</a>`;
        } else {
            const rawSize = Number(item.size);
            totalsize = totalsize + rawSize;
            item['size'] = formatFileSize(item['size']);
            is_file = true;
            const epn = item.name;
            const link = UI.second_domain_for_dl ? UI.downloaddomain + item.link : window.location.origin + item.link;
            ...
            pn += "?a=view";
            c += " view";
            const copyBtn = `<button class="btn btn-sm gdi-copy-btn float-end ms-1" onclick="copyShareUrl('${window.location.origin + pn}')" title="Copy link"><i class="bi bi-link-45deg"></i></button>`;
            html += `<div class="list-group-item list-group-item-action gdi-file-item" data-name="${item.name.toLowerCase()}" data-bytes="${rawSize}" data-date="${item['modifiedTime'] || ''}">${UI.allow_selecting_files ? '<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="'+link+'" id="flexCheckDefault">' : ''}`;

            // Use centralized getFileIcon helper
            html += getFileIcon(ext);

            html += ` <a class="countitems size_items list-group-item-action" style="text-decoration: none; color: ${UI.css_a_tag_color};" href="${pn}">${item.name}</a>${copyBtn}${UI.display_download ? `<a href="${link}"><svg class="float-end"width="25px" style="margin-left: 8px;" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"> <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"></path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path> </svg></a>` : ``}${UI.display_size ? `<span class="badge bg-primary float-end"> ` + item['size'] + ` </span>` : ``}${UI.display_time ? ` <span class="badge bg-info float-end"> ` + item['modifiedTime'] + ` </span>` : ``}</div>`;
        }
    }
    if (is_file && UI.allow_selecting_files) {
        document.getElementById('select_items').style.display = 'block';
    ...
    // When it is page 1, remove the horizontal loading bar
    $list.html(($list.data('curPageIndex') == '0' ? '' : $list.html()) + html);
    // Initialize folder filter and column sort when list is ready
...
async function fallback(id, type) {
    if (type) { // is a file id
        ...
        fetch("/0:fallback", {
            ...
            .then(function(obj) {
                dispatchFileView(obj, cookie_folder_id);
            })
            .catch(function(error) {
...
// ── Shared file-view helpers ──────────────────────────────────────────────────

/**
 * Route a file metadata object to the correct preview handler.
 * Used by both fallback() and file() to avoid duplication.
 */
...
/**
 * Render a standardised error card HTML string.
 */
function renderErrorCard(error) {
    return `<div class="container mt-3">
      <div class="card text-center">
        <div class="card-body">
          <div class="${UI.file_view_alert_class}" role="alert"><b>Error.</b> ${error}</div>
        </div>
        <p class="card-text">The requested resource could not be loaded.</p>
        <div class="card-body">
...
/**
 * Render the download / open-in-app button group HTML.
 * @param {string} url         - Download URL
 * @param {string} encoded_name - URL-encoded filename
 * @param {object} opts        - { showMedia: bool } — include media player intents
 */
function renderDownloadButtons(url, encoded_name, opts = {}) {
    ...
    return `<div class="input-group mb-4">
        <input type="text" class="form-control" id="dlurl" value="${url}" readonly>
        <button class="btn btn-outline-secondary" type="button" onclick="copyShareUrl('${url}')" title="Copy link"><i class="bi bi-clipboard"></i></button>
      </div>
      <div class="btn-group text-center">
        <a href="${url}" type="button" class="btn btn-primary"><i class="bi bi-download me-1"></i>Download</a>
        <button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-expanded="false"><span class="visually-hidden">More options</span></button>
        <ul class="dropdown-menu">
          ${mediaItems}
          <li><a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${encoded_name};end">1DM (Free)</a></li>
          <li><a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${encoded_name};end">1DM (Lite)</a></li>
          <li><a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${encoded_name};end">1DM+ (Plus)</a></li>
        </ul>
      </div>`;
}

/**
 * Copy a URL to the clipboard and show a brief toast notification.
 */
...
/**
 * Show a brief Bootstrap toast message.
 */
function showToast(message) {
    ...
    container.insertAdjacentHTML('beforeend', `
      <div id="${id}" class="toast align-items-center text-bg-dark border-0" role="alert" aria-live="assertive">
        <div class="d-flex">
...
function file_others(name, encoded_name, size, url, file_id, cookie_folder_id) {
    const navigation = generateBreadcrumb(window.location.pathname);
    const content = `
    <div class="container mt-3">
      <nav aria-label="breadcrumb"><ol class="breadcrumb">${navigation}</ol></nav>
      <div class="card text-center">
        <div class="card-body">
          <div class="${UI.file_view_alert_class}" role="alert">${name}<br>${size}</div>
        </div>
        <div class="card-body">
...
function file_code(name, encoded_name, size, bytes, url, ext, file_id, cookie_folder_id) {
    ...
    <div class="container mt-3">
      ...
      <div class="card text-center">
        ...
        ${UI.second_domain_for_dl ? '' : '<pre style="white-space:pre-wrap;text-align:left;padding:1rem;"><code id="editor"></code></pre>'}
        <div class="card-body">
          ${renderDownloadButtons(url, encoded_name)}
          <br>
        </div>
    ...
    if (!UI.second_domain_for_dl) {
        $('#code_spinner').html('<div class="d-flex justify-content-center"><div class="spinner-border m-5" role="status"></div></div>');
        if (bytes <= 1024 * 1024 * 2) {
            ...
            $('#code_spinner').html('');
            $('#editor').html(`<div class="${UI.file_view_alert_class}" role="alert">File too large to preview (max 2 MB)</div>`);
        }
    }
}
...
function file_video(name, encoded_name, size, poster, url, mimeType, file_id, cookie_folder_id, subtitles) {
    ...
    if (!UI.disable_player) {
        if (player_config.player === 'plyr') {
            ...
            playerHtml = `<video id="vplayer" poster="${poster}" muted class="video-js vjs-default-skin vjs-big-play-centered" controls preload="auto" width="100%" height="100%" data-setup='{"fluid":true}'>
              <source src="${url}" type="${isHLS ? 'application/x-mpegURL' : 'video/mp4'}">
    ...
    <div class="container mt-3">
      ...
      <div class="card text-center">
        <div class="card-body">
          <div class="${UI.file_view_alert_class}" role="alert">${name}<br>${size}</div>
          <div class="gdi-player-wrap">${playerHtml}</div>
        </div>
        ${UI.disable_video_download ? '' : `
        <div class="card-body">
          ${renderDownloadButtons(url, encoded_name, { showMedia: true })}
          <br>
        </div>`}
      </div>
    </div>`;
    ...
    if (player_js) {
        ...
        script.onload = function() {
            if (player_config.player === 'plyr') {
                ...
                // Hotkeys: space=pause, f=fullscreen, m=mute, arrows=seek/vol
                vjsPlayer.ready(function() {
                    this.el().addEventListener('keydown', function(e) {
                        if (e.target.tagName === 'INPUT') return;
                        if (e.key === ' ') { e.preventDefault(); vjsPlayer.paused() ? vjsPlayer.play() : vjsPlayer.pause(); }
                        else if (e.key === 'f') vjsPlayer.isFullscreen() ? vjsPlayer.exitFullscreen() : vjsPlayer.requestFullscreen();
                        else if (e.key === 'm') vjsPlayer.muted(!vjsPlayer.muted());
                        else if (e.key === 'ArrowRight') vjsPlayer.currentTime(vjsPlayer.currentTime() + 10);
                        else if (e.key === 'ArrowLeft') vjsPlayer.currentTime(Math.max(0, vjsPlayer.currentTime() - 10));
                        else if (e.key === 'ArrowUp') vjsPlayer.volume(Math.min(1, vjsPlayer.volume() + 0.1));
                        else if (e.key === 'ArrowDown') vjsPlayer.volume(Math.max(0, vjsPlayer.volume() - 0.1));
                    });
                });
            } else if (player_config.player === 'dplayer') {
                new DPlayer({
...
// File display Audio |mp3|flac|m4a|wav|ogg| — uses APlayer for a proper audio experience
function file_audio(name, encoded_name, size, url, file_id, cookie_folder_id, playlist) {
    ...
    <div class="container mt-3">
      ...
      <div class="card text-center">
        <div class="card-body">
          <div class="${UI.file_view_alert_class}" role="alert">${name}<br>${size}</div>
          ${UI.disable_player ? '' : '<div id="aplayer-container" style="max-width:680px;margin:0 auto;"></div>'}
        </div>
    ...
    // Optionally try to load sibling audio files to build a playlist
    // (worker returns them via the parent folder listing)
    if (playlist.length <= 1) {
        ...
        requestListPath(parentPath, {}, function(res) {
            ...
            if (audioFiles.length > 1) {
                ...
                // Reinit APlayer with full playlist
...
// Document display PDF — uses PDF.js canvas renderer (no Google Docs Viewer dependency)
function file_pdf(name, encoded_name, size, url, file_id, cookie_folder_id) {
    ...
    <div class="container mt-3">
      ...
      <div class="card">
        <div class="card-body text-center">
          <div class="${UI.file_view_alert_class}" role="alert">${name}<br>${size}</div>
          <div id="pdf-controls" class="my-2 d-flex flex-wrap align-items-center justify-content-center gap-2">
            <button id="pdf-prev" class="btn btn-sm btn-secondary">&#8249; Prev</button>
            ...
            <input id="pdf-zoom" type="range" min="50" max="200" value="100" style="width:120px;" title="Zoom">
            <span id="pdf-zoom-val">100%</span>
          </div>
          <div id="pdf-spinner" class="d-flex justify-content-center my-4">
            <div class="spinner-border ${UI.loading_spinner_class}" role="status"></div>
          </div>
...
function file_image(name, encoded_name, size, url, file_id, cookie_folder_id) {
    const navigation = generateBreadcrumb(window.location.pathname);
    const dlButtons = renderDownloadButtons(url, encoded_name);
    const content = `
    <div class="container text-center"><br>
      <nav aria-label="breadcrumb"><ol class="breadcrumb">${navigation}</ol></nav>
      <div class="card">
        <div class="card-body text-center">
          <div class="${UI.file_view_alert_class}" id="file_details" role="alert">${name}<br>${size}</div>
          <img src="${url}" id="load_image" class="img-fluid" alt="${name}">
        </div>
        <div class="card-body">
          <div class="input-group mb-3">
            <span class="input-group-text">Full URL</span>
            <input type="text" class="form-control" id="dlurl" value="${url}" readonly>
            <button class="btn btn-outline-secondary gdi-copy-btn" type="button"
              onclick="copyShareUrl('${url}')" title="Copy URL">&#x2398;</button>
          </div>
          ${dlButtons}
        </div>
      </div>
...
// Time conversion — uses browser locale and timezone
function formatDateTime(utc_datetime) {
    if (!utc_datetime) return '';
    return new Date(utc_datetime).toLocaleString();
}
// Alias kept so callers don't need updating all at once
...
// Trim a specific character from both ends of a string (standalone, does NOT override built-in .trim())
...
$(function() {
    init();
    // Embed mode: hide navbar and footer
