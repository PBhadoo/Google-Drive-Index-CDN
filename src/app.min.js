...
</nav>`;$("#nav").html(s),applyTheme(localStorage.getItem("gdi-theme")||"dark")}function render(i){i.indexOf("?")>=0&&(i=i.substr(0,i.indexOf("?"))),title(i),nav(i);const e=/\/\d+:$/g;if(i.includes("/fallback")){window.scroll_status={event_bound:!1,loading_lock:!1};const t=getQueryVariable("a"),n=getQueryVariable("id");return t?fallback(n,!0):list(null,n,!0)}else window.MODEL.is_search_page?(window.scroll_status={event_bound:!1,loading_lock:!1},render_search_result_list()):i.match(e)||i.slice(-1)=="/"?(window.scroll_status={event_bound:!1,loading_lock:!1},list(i)):file(i)}function requestListPath(i,e,t,n,a=3,c=!1){const l={id:e.id||"",type:"folder",password:e.password||"",page_token:e.page_token||"",page_index:e.page_index||0},d=a!=null&&a>=0?a:3;$("#update").show(),document.getElementById("update").innerHTML='<div class="gdi-alert gdi-alert-info">Connecting\u2026</div>',c&&(i="/0:fallback");async function o(s){try{const p=await fetch(c?"/0:fallback":i,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)});if(!p.ok)throw new Error("Request failed with status "+p.status);const r=await p.json();r&&r.error&&r.error.code===401?($("#update").hide(),askPassword(i)):r&&r.data===null?(document.getElementById("spinner")?.remove(),document.getElementById("list").innerHTML=`<div class="gdi-empty"><i class="bi bi-exclamation-circle"></i><p>Server didn't send any data.</p></div>`,$("#update").hide()):r&&r.data&&(t(r,i,l),$("#update").hide())}catch(p){s>0?(document.getElementById("update").innerHTML=`<div class="gdi-alert gdi-alert-info">Retrying\u2026 (${s} left)</div>`,await sleep(2e3),await o(s-1)):(document.getElementById("update").innerHTML='<div class="gdi-alert gdi-alert-error">Unable to connect. Please try again.</div>',document.getElementById("list").innerHTML=`<div class="gdi-empty"><i class="bi bi-wifi-off"></i><p>${escHtml(String(p))}</p></div>`,$("#update").hide())}}o(d)}function requestSearch(i,e,t=3){const n={q:i.q||null,page_token:i.page_token||null,page_index:i.page_index||0};async function a(c){try{const l=await fetch(`/${window.current_drive_order}:search`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!l.ok)throw new Error("Request failed with status "+l.status);const d=await l.json();d&&d.data===null?($("#spinner").remove(),$("#list").html('<div class="gdi-empty"><i class="bi bi-search"></i><p>No results found.</p></div>'),$("#update").remove()):d&&d.data&&(e&&e(d,n),$("#update").remove())}catch{c>0?($("#update").html(`<div class="gdi-alert gdi-alert-info">Retrying\u2026 (${c} left)</div>`),await sleep(2e3),await a(c-1)):($("#update").html(`<div class="gdi-alert gdi-alert-error">Unable to connect after ${t} attempts.</div>`),$("#list").html('<div class="gdi-empty"><i class="bi bi-wifi-off"></i><p>Connection failed.</p></div>'),$("#spinner").remove())}}$("#update").html('<div class="gdi-alert gdi-alert-info">Searching\u2026</div>'),a(t)}function list(i,e="",t=!1){const n=window.location.pathname,a=trimChar(n,"/").split("/");let c="/",l='<li><a href="/">Home</a></li><li class="gdi-bc-sep">/</li>';if(a.length>1)for(const r in a){const g=a[r];let f;try{f=decodeURIComponent(g)}catch{f=g}f=f.replace(/\//g,"%2F");const u=f.length>18?f.slice(0,14)+"\u2026":f;if(c+=g+"/",!u)break;Number(r)===a.length-1?l+=`<li class="gdi-bc-cur">${escHtml(u)}</li>`:l+=`<li><a href="${c}">${escHtml(u)}</a></li><li class="gdi-bc-sep">/</li>`}const d=`
<div class="gdi-wrap">
  <div id="update"></div>
...
</div>`}}if(l&&UI.allow_selecting_files&&(document.getElementById("select_items").style.display="flex"),t.html((t.data("curPageIndex")=="0"?"":t.html())+a),n){const d=formatFileSize(c)||"0 Bytes",o=t.find(".countitems").length,s=t.find(".size_items").length,p=o===0?"Empty folder":`${o} item${o===1?"":"s"}`,r=s>0?` &middot; ${s} file${s===1?"":"s"}, ${d}`:"";$("#count").addClass("show").html(p+r)}}catch(t){console.error(t)}}function render_search_result_list(){const i=window.MODEL.q||"",e=`
<div class="gdi-wrap">
  <div id="update"></div>
...
`);navigator.clipboard.writeText(c).catch(()=>{const l=document.createElement("textarea");l.value=c,document.body.appendChild(l),l.select(),document.execCommand("copy"),document.body.removeChild(l)}),showToast(`${a.length} link${a.length>1?"s":""} copied`)})}function append_search_result_to_list(i){try{const e=$("#list"),t=e.data("nextPageToken")===null;let n="",a=0,c=!1;for(const l in i){const d=i[l];d.size==null&&(d.size=""),d.modifiedTime=utc2delhi(d.modifiedTime);const o=typeof d.rootIdx=="number"?d.rootIdx:-1;if(d.mimeType=="application/vnd.google-apps.folder")n+=`<a style="cursor:pointer;" onclick="onSearchResultItemClick('${escHtml(d.id)}', false, ${o})" data-bs-toggle="modal" data-bs-target="#SearchModel"
  ...
  <span class="gdi-row-acts"></span>
</a>`;else{c=!0;const s=Number(d.size);a+=s,d.size=formatFileSize(d.size);const p=d.fileExtension,r=UI.second_domain_for_dl?UI.downloaddomain+d.link:window.location.origin+d.link;n+=`<div class="gdi-row countitems size_items" data-name="${escHtml(d.name.toLowerCase())}" data-bytes="${s}" data-date="${d.modifiedTime||""}">
  ${UI.allow_selecting_files?`<input class="gdi-row-check" type="checkbox" value="${r}">`:""}
  <span class="gdi-row-icon">${getFileIcon(p)}</span>
  <span class="gdi-row-name" onclick="onSearchResultItemClick('${escHtml(d.id)}', true, ${o})" data-bs-toggle="modal" data-bs-target="#SearchModel" style="cursor:pointer;">${escHtml(d.name)}</span>
...
</div>`}}if(c&&UI.allow_selecting_files&&(document.getElementById("select_items").style.display="flex"),e.html((e.data("curPageIndex")=="0"?"":e.html())+n),t){const l=formatFileSize(a)||"0 Bytes",d=e.find(".countitems").length,o=e.find(".size_items").length;if(d===0)$("#count").addClass("show").html("No results found");else{const s=`${d} result${d===1?"":"s"}`,p=o>0?` &middot; ${l}`:"";$("#count").addClass("show").html(s+p)}}}catch(e){console.error(e)}}function onSearchResultItemClick(i,e,t){$("#SearchModelLabel").html("Loading\u2026"),$("#modal-body-space").html('<div class="gdi-spinner-wrap"><div class="gdi-spinner"></div></div>');const n=typeof t=="number"&&t>=0?t:0;async function a(){try{const c=await fetch(`/${n}:id2path`,{method:"POST",body:JSON.stringify({id:i}),headers:{"Content-Type":"application/json"}});if(c.ok){const l=await c.json();if(l.path){const d=l.path.replace(/#/g,"%23").replace(/\?/g,"%3F");$("#SearchModelLabel").html("Open file"),$("#modal-body-space").html(`
                      <a class="gdi-btn gdi-btn-primary me-2" href="${d}${e?"?a=view":""}">Open</a>
                      <a class="gdi-btn gdi-btn-ghost" href="${d}${e?"?a=view":""}" target="_blank">Open in new tab</a>`);return}}}catch{}$("#SearchModelLabel").html("Not in configured drives"),$("#modal-body-space").html(`
